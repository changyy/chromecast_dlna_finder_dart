...
class DiscoveryService {
  ...
  Future<Map<String, List<DiscoveredDevice>>> discoverAllDevices({Duration timeout = const Duration(seconds: 5)}) async {
    ...
    try {
      ...
      final chromecastTask = scanChromecastDevices().then((devices) {
        result['chromecast'] = devices;
      }).catchError((e) async {
        await _logger.error('errors.chromecast_scan_error', tag: 'Discovery', error: e, params: {'error': e.toString()});
      ...
      final dlnaTask = scanAllDlnaDevices(timeout: timeout).then((devices) {
        result['dlna'] = devices;
      }).catchError((e) async {
        await _logger.error('errors.dlna_scan_error', tag: 'Discovery', error: e, params: {'error': e.toString()});
      ...
      await _logger.error('errors.unexpected_scan_error', tag: 'Discovery', error: e, params: {'error': e.toString()});
    ...
    result['errors'] = errors.map((e) => DiscoveredDevice(
      name: e,
      ip: '',
      type: DeviceType.unknown,
    )).toList();
    
    await _logger.debug('Device discovery complete: Chromecast=${result['chromecast']!.length}, DLNA=${result['dlna']!.length}, Errors=${errors.length}', tag: 'Discovery');
  ...
  Map<String, dynamic> toJson(Map<String, List<DiscoveredDevice>> result) {
    ...
    json['chromecast'] = result['chromecast']!.map((device) => device.toJson()).toList();
    json['chromecast_dongle'] = result['chromecast_dongle']!.map((device) => device.toJson()).toList();
    json['chromecast_audio'] = result['chromecast_audio']!.map((device) => device.toJson()).toList();
    json['dlna'] = result['dlna']!.map((device) => device.toJson()).toList();
    json['dlna_renderer'] = result['dlna_renderer']!.map((device) => device.toJson()).toList();
    json['dlna_media_server'] = result['dlna_media_server']!.map((device) => device.toJson()).toList();

    json['count'] = {
      'tx': result['dlna_media_server']!.length,
      'rx': result['chromecast']!.length + result['dlna_renderer']!.length,
      'total': result['chromecast']!.length + result['dlna_renderer']!.length + result['dlna_media_server']!.length,
    };

    // Process errors
    json['error'] = result['errors']?.map((device) => device.name).toList() ?? [];
