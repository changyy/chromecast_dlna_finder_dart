...
void main() {
  group('Localization resource file tests', () {
    test('English language file integrity test', () async {
      ...
      expect(await file.exists(), isTrue, reason: 'English language file should exist');
    ...
    test('Traditional Chinese language file integrity test', () async {
      ...
      expect(await file.exists(), isTrue, reason: 'Traditional Chinese language file should exist');
    ...
    test('English and Chinese key consistency test', () async {
      ...
      final zhTwFile = File('${Directory.current.path}/lib/src/locales/zh_TW.json');
      ...
      for (final key in enJson.keys) {
        expect(zhTwJson.containsKey(key), isTrue, reason: 'Traditional Chinese file missing top-level key: $key');
      }
      ...
      for (final key in enInfo.keys) {
        expect(zhTwInfo.containsKey(key), isTrue, reason: 'Traditional Chinese file missing info section key: $key');
      }
      ...
      for (final key in enErrors.keys) {
        expect(zhTwErrors.containsKey(key), isTrue, reason: 'Traditional Chinese file missing errors section key: $key');
      }
    });
  });
}