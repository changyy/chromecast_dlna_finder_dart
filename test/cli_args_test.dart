  ...
  group('Command line interface tests', () {
    ...
    setUp(() {
      ...
      parser = ArgParser()
        ...
        ..addOption('lang', abbr: 'l', help: 'Specify language (en, zh-tw)', defaultsTo: '')
        ..addOption('timeout', abbr: 't', help: 'Scan timeout in seconds', defaultsTo: '15');
    });
    ...
    test('Option combination test', () {
      final results = parser.parse(['--debug', '--quiet', '--minify', '--timeout', '30']);
    ...
    test('Invalid parameter test', () {
      expect(() => parser.parse(['--invalid']), throwsA(isA<FormatException>()));
    });
  });
}